  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="ios-card ios-blur rounded-2xl p-4 ios-shadow-lg border-0">
        ...
        <p className="text-gray-600 font-medium">Mean: {data.value?.toFixed(3)} {data.unit}</p>
        {data.n !== undefined && <p className="text-gray-500 text-sm">Samples (n): {data.n}</p>}
      </div>
    );
  }
  return null;
...
export default function ChartVisualizationEn({ samples }) {
  if (samples.length === 0) {
    return (
      <Card className="ios-card ios-blur rounded-3xl ios-shadow-lg border-0">
        <CardContent className="p-8 text-center">
          <p className="text-gray-500 font-medium text-lg">No data to visualize</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <CardContent className="space-y-6 pt-6">
          <div className="p-6 rounded-2xl bg-white/80 ios-shadow border border-gray-100/50 ios-blur">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-center">
              <div>
                <p className="text-gray-500 text-sm font-semibold mb-1">Total Samples</p>
                <p className="text-gray-900 font-bold text-2xl">{samples.length}</p>
              </div>
              <div>
                <p className="text-gray-500 text-sm font-semibold mb-1">Total Treatments</p>
